...
def test_category_filtering(token, session_id):
    ...
    if response.status_code == 200:
        ...
        results = result.get('results', [])
        if results:
            print(f"✅ Category filtering successful - found {len(results)} results")
            categories = [r.get('category', 'N/A') for r in results[:3]]
            print(f"   Sample categories: {categories}")
            return True
        else:
            print("❌ Category filtering returned no results")
            return False
    else:
...
def test_json_upload(token):
    ...
    # Create test product data - should be a direct array, not wrapped
    test_products = [
        {
    ...
    # Convert to file upload format expected by the API - direct JSON array
    files = {'file': ('products.json', json.dumps(test_products), 'application/json')}
    ...
    if response.status_code == 200:
        ...
        if result.get('status') == 'success':
            ...
            print(f"❌ JSON upload failed: {result.get('message', 'Unknown error')}")
            return False
    else:
...
def test_image_search(token, session_id):
    ...
    data = {'query': 'red colored item'}
    
    response = requests.post('http://localhost:8000/products/search', files=files, data=data, headers=headers)
...
def main():
    ...
    # Test text search
    if test_text_search(token, session_id):
        tests_passed += 1
    
    # Test category filtering
    if test_category_filtering(token, session_id):
        tests_passed += 1
    ...
    # Test image search
    if test_image_search(token, session_id):
        tests_passed += 1
