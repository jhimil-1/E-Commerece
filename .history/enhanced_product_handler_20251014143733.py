...
class EnhancedProductHandler:
    ...
    def filter_irrelevant_results(self, query: str, products: List[Dict[str, Any]], 
        ...
        if min_semantic_score is None:
            ...
            elif any(word in query_lower for word in ['jewelry', 'jewellery', 'jewellry', 'earrings', 'necklace', 'bracelet', 'ring', 'watch']):
            ...
            elif any(word in query_lower for word in ['clothing', 'dress', 'shirt', 'pants', 'jeans', 'skirt', 'top', 'blouse']):
                # For clothing queries, be more lenient to allow clothing-related items
                min_semantic_score = 0.1  # Lower threshold for clothing queries
                print(f"DEBUG: Clothing detected, setting min_semantic_score to 0.1")
            else:
        ...
        print(f"DEBUG: Starting loop over {len(products)} products")
        ...
        for i, product in enumerate(products):
            ...
            # CRITICAL: Check for headphones queries FIRST, before phone queries
            # This prevents "phone" in "headphones" from triggering phone logic
            headphones_words = ['headphones', 'headphone', 'earbuds', 'earphone']
            is_headphones_query = any(word in query_lower for word in headphones_words)
            
            if is_headphones_query:
                ...
                if semantic_score >= min_semantic_score and vector_score >= self.min_relevance_score * 0.5:
                    ...
                    continue  # Explicitly skip this product
                continue  # Skip to next product after headphones processing
            
            # For phone queries, prioritize vector score over semantic score for electronics
            # Note: This won't trigger for headphones queries due to the continue above
            if any(word in query_lower for word in ['phone', 'smartphone', 'mobile', 'cell', 'telephone']):
                print(f"DEBUG: Phone query detected, checking if electronics: {is_electronics}, has_decent_vector: {has_decent_vector}")
                if is_electronics and has_decent_vector:
                    # Allow electronics products with decent vector scores for phone queries
                    product['enhanced_score'] = combined_score
                    product['semantic_relevance'] = semantic_score
                    filtered_products.append(product)
                    print(f"DEBUG: Phone query - adding {product.get('name', 'Unknown')} and continuing")
                    continue
            
            # For jewelry queries, prioritize vector score over semantic score for jewelry
            if any(word in query_lower for word in ['jewelry', 'jewellery', 'jewellry', 'earrings', 'necklace', 'bracelet', 'ring', 'watch']):
                if is_jewelry and has_decent_vector:
                    # Allow jewelry products with decent vector scores for jewelry queries
                    product['enhanced_score'] = combined_score
                    product['semantic_relevance'] = semantic_score
                    filtered_products.append(product)
                    logger.debug(f"JEWELRY PASSED: {product.get('name', 'Unknown')} "
                               f"(vector: {vector_score:.3f}, semantic: {semantic_score:.3f}, "
                               f"combined: {combined_score:.3f})")
                    continue
                # For other jewelry queries, apply standard filtering
                if vector_score >= self.min_relevance_score and semantic_score >= min_semantic_score:
                    # Update the product with enhanced scores
                    product['enhanced_score'] = combined_score
                    product['semantic_relevance'] = semantic_score
                    filtered_products.append(product)
                    logger.debug(f"JEWELRY STANDARD: {product.get('name', 'Unknown')} "
                               f"(vector: {vector_score:.3f}, semantic: {semantic_score:.3f}, "
                               f"min_vector: {self.min_relevance_score}, min_semantic: {min_semantic_score})")
                else:
                    logger.debug(f"JEWELRY FILTERED OUT: {product.get('name', 'Unknown')} "
                               f"(vector: {vector_score:.3f}, semantic: {semantic_score:.3f}, "
                               f"min_vector: {self.min_relevance_score}, min_semantic: {min_semantic_score})")
            
            # For clothing queries, prioritize vector score over semantic score for clothing
            if any(word in query_lower for word in ['clothing', 'dress', 'shirt', 'pants', 'jeans', 'skirt', 'top', 'blouse']):
                if product_category == 'clothing' and has_decent_vector:
                    # Allow clothing products with decent vector scores for clothing queries
                    product['enhanced_score'] = combined_score
                    product['semantic_relevance'] = semantic_score
                    filtered_products.append(product)
                    logger.debug(f"CLOTHING PASSED: {product.get('name', 'Unknown')} "
                               f"(vector: {vector_score:.3f}, semantic: {semantic_score:.3f}, "
                               f"combined: {combined_score:.3f})")
                    continue
                # For other clothing queries, apply standard filtering
                if vector_score >= self.min_relevance_score and semantic_score >= min_semantic_score:
                    # Update the product with enhanced scores
                    product['enhanced_score'] = combined_score
                    product['semantic_relevance'] = semantic_score
                    filtered_products.append(product)
                    logger.debug(f"CLOTHING STANDARD: {product.get('name', 'Unknown')} "
                               f"(vector: {vector_score:.3f}, semantic: {semantic_score:.3f}, "
                               f"min_vector: {self.min_relevance_score}, min_semantic: {min_semantic_score})")
                else:
                    logger.debug(f"CLOTHING FILTERED OUT: {product.get('name', 'Unknown')} "
                               f"(vector: {vector_score:.3f}, semantic: {semantic_score:.3f}, "
                               f"min_vector: {self.min_relevance_score}, min_semantic: {min_semantic_score})")
