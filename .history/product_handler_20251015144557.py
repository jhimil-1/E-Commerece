            ...
            if normalized_user_id:
                try:
                    ...
                    if self._is_valid_objectid(normalized_user_id):
                        ...
                        user = self.db.users.find_one({"_id": ObjectId(normalized_user_id)}).hint("_id_")
                    ...
                    if not user and '-' in normalized_user_id:
                        # Try UUID lookup
                        user = self.db.users.find_one({"user_id": normalized_user_id}).hint("user_id_1")
                        logger.debug(f"Tried UUID lookup for {normalized_user_id}: {'found' if user else 'not found'}")
                    
                    if not user:
                        # Fallback to username lookup
                        user = self.db.users.find_one({"username": normalized_user_id}).hint("username_1")
