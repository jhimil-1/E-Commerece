...
def test_text_search(token):
    ...
    response = requests.post('http://localhost:8000/api/search', json=search_data, headers=headers)
...
def test_category_filtering(token):
    ...
    response = requests.post('http://localhost:5000/api/search', json=search_data, headers=headers)
    ...
    if response.status_code == 200:
        ...
        if result.get('success'):
            ...
            print(f"❌ Category filtering failed: {result.get('error')}")
            return False
    else:
...
def test_json_upload(token):
    ...
    response = requests.post('http://localhost:5000/api/upload-products', json=upload_data, headers=headers)
...
def test_image_search(token):
    ...
    response = requests.post('http://localhost:5000/api/search-by-image', files=files, data=data, headers=headers)
    ...
    if response.status_code == 200:
        ...
        if result.get('success'):
            ...
            print(f"❌ Image search failed: {result.get('error')}")
            return False
    else:
