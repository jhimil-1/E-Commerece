...
class ProductHandler:
    ...
    async def search_products(
        ...
        try:
            ...
            # Get user info from user_id (could be MongoDB ObjectId or UUID) for Qdrant filtering
            try:
            ...
            # Use the actual user_id from the database for Qdrant filtering, not username
            # Handle both old users (with _id only) and new users (with user_id field)
            if user:
                qdrant_user_id = user.get("user_id", str(user["_id"]))
            else:
                qdrant_user_id = user_id
            
            # Search in Qdrant with user filtering and category filtering
            logger.info(f"Searching Qdrant with embedding size: {len(query_embedding)}, user: {qdrant_user_id}, category: {category}, limit: {limit * 2}, min_score: {min_score}")
            search_results = qdrant_manager.search_similar_products(
                query_embedding=query_embedding,
                user_id=qdrant_user_id,  # Use actual user_id for Qdrant filtering
                category_filter=category,
                limit=limit * 2,  # Get more results to filter further
                min_score=min_score  # Pass min_score parameter
            )
            ...
            logger.info(f"Retrieving products from MongoDB with IDs: {product_ids}")
            ...
            for product in products_cursor:
                ...
                # Keep all products visible for shared catalog - removed user filtering
                # Products from all users are available in the shared catalog
                
                products_dict[product_id] = product
                
            logger.info(f"Retrieved {len(products_dict)} products from MongoDB")
