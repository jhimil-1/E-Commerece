...
class ProductHandler:
    ...
    async def search_products(
        ...
        try:
            ...
            # Get username from user_id (could be MongoDB ObjectId or UUID) for Qdrant filtering
            try:
            ...
            logger.info(f"Searching Qdrant with embedding size: {len(query_embedding)}, user: {username}, category: {category}, limit: {limit * 2}, min_score: {min_score}")
            search_results = qdrant_manager.search_similar_products(
                query_embedding=query_embedding,
                user_id=username,  # Use username for Qdrant filtering
