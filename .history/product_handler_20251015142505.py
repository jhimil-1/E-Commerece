...
class ProductHandler:
    ...
    async def search_products(
        ...
        """
        Search for products using CLIP-based similarity on text and/or image.
        ...
        """
        try:
            logger.info(f"Starting product search - Query: '{query}', Category: {category}, User: {user_id}, Limit: {limit}")
            
            # Auto-detect category from query if not explicitly provided
            if not category and query:
                query_lower = query.lower()
                clothing_keywords = ["clothes", "clothing", "dress", "apparel", "shirt", "pants", "jeans", "jacket", "coat", "skirt", "blouse", "sweater"]
                electronics_keywords = ["electronics", "tech", "gadget", "device", "smart", "digital", "computer", "phone", "tablet", "laptop"]
                jewelry_keywords = ["jewelry", "jewellery", "earrings", "necklace", "bracelet", "ring", "watch", "pendant", "chain"]
                
                # Check for clothing-related terms
                for keyword in clothing_keywords:
                    if keyword in query_lower:
                        category = "clothing"
                        logger.info(f"Auto-detected clothing category from query: {query}")
                        break
                ...
                if not category:
                    for keyword in electronics_keywords:
                        if keyword in query_lower:
                            ...
                            logger.info(f"Auto-detected electronics category from query: {query}")
                            break
                
                # Check for jewelry-related terms
                if not category:
                    for keyword in jewelry_keywords:
                        if keyword in query_lower:
                            category = "jewelry"
                            logger.info(f"Auto-detected jewelry category from query: {query}")
                            break
            ...
            if user_id:
                ...
                try:
                    # Check if user_id looks like a valid ObjectId (24 hex characters)
                    if len(user_id) == 24 and all(c in '0123456789abcdefABCDEF' for c in user_id):
                        ...
                        if not user:
                            # If ObjectId lookup fails, try by user_id (for new users)
                            user = self.db.users.find_one({"user_id": user_id})
                    else:
                        # For UUID format (new users), try by user_id directly
                        user = self.db.users.find_one({"user_id": user_id})
