...
class UIController {
    ...
    updateUI() {
        ...
        if (isAuthenticated) {
            ...
            uploadSection.style.display = 'block'; // Show upload section for authenticated users
            ...
            uploadSection.style.display = 'none'; // Hide upload section for unauthenticated users
    ...
    async uploadJsonProducts() {
        ...
        try {
            const result = await this.api.uploadProducts(this.jsonData);
    ...
    async handleJSONUpload(event) {
        ...
        try {
            ...
            this.showStatus(`JSON file loaded: ${Array.isArray(this.jsonData.products) ? this.jsonData.products.length : 'Unknown'} products found`, 'success');
        } catch (error) {
