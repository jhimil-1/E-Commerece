    ...
    try:
        ...
        if is_jewelry_search:
            search_results = await enhanced_product_handler.search_jewelry_by_image_and_category(
                ...
                category=category,
                limit=10,
                min_score=0.4 # Higher threshold for only highly relevant jewelry results
            )
        else:
            ...
            search_results = await enhanced_product_handler.search_products(
                ...
                category=category,
                limit=10,
                min_relevance_score=0.3,  # Higher threshold for only highly relevant results
                search_type="image",
                user_id=current_user.get("user_id", current_user.get("username"))
            )
...
async def product_similarity_search(
    ...
    try:
        ...
        results = await enhanced_product_handler.search_products(
            ...
            category=category,
            limit=limit,
            min_relevance_score=0.6,  # Higher threshold for only highly relevant results
            search_type="image",
            user_id=current_user.get("user_id", current_user.get("username"))
        )
