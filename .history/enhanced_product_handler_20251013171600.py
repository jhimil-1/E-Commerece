...
class EnhancedProductHandler:
    ...
    def calculate_semantic_relevance(self, query: str, product: Dict[str, Any]) -> float:
        ...
        # Penalize obviously wrong categories
        ...
        for keyword, wrong_cats in wrong_category_penalties.items():
            if keyword in query_lower and product_category in wrong_cats:
                relevance_score -= 0.5  # Heavy penalty for wrong category
    ...
    def filter_irrelevant_results(self, query: str, products: List[Dict[str, Any]], 
        ...
        if min_semantic_score is None:
            # Lower threshold for electronics/broad queries
            if any(word in query.lower() for word in ['electronics', 'phone', 'smartphone', 'tech', 'gadget']):
                min_semantic_score = 0.1  # More lenient for electronics
            else:
                min_semantic_score = 0.3  # Standard threshold for clothing/etc
        ...
        for product in products:
            ...
            # Only include products that meet both thresholds
