...
async def debug_headphones_filtering():
    ...
    try:
        ...
        for i, product in enumerate(filtered_result):
            ...
            # Debug: Show what terms are being matched
            query_lower = "show me headphones"
            product_name_lower = name
            product_desc = description
            product_category = product.get('category', '').lower()
            
            print(f"  {i+1}. {product['name']} (${product['price']})")
            print(f"     Category: {product.get('category', 'unknown')}")
            print(f"     Query: '{query_lower}' -> Name: '{product_name_lower}'")
            print(f"     Desc: '{product_desc[:50]}...'")
