...
def test_upload_with_image():
    ...
    if response.status_code == 200:
        ...
        print(f"Upload successful! Full response: {result}")
        print(f"Inserted count: {result.get('details', {}).get('inserted_count', 0)}")
        print(f"Product IDs: {result.get('details', {}).get('product_ids', [])}")
        return result.get('details', {}).get('product_ids', [])[0] if result.get('details', {}).get('product_ids') else None
    else:
...
def test_search_for_product():
    ...
    if search_response.status_code == 200:
        ...
        if products:
            ...
            print(f"Image URL: {first_product.get('image_url', 'N/A')[:50]}..." if first_product.get('image_url') else "Image URL: N/A")
            print(f"Image Path: {first_product.get('image_path', 'N/A')}")
            print(f"Image: {first_product.get('image', 'N/A')}")
            print(f"Payload keys: {list(first_product.get('payload', {}).keys())}")
    else:
