...
class EnhancedProductHandler:
    ...
    async def search_jewelry_by_image_and_category(self, 
        ...
        # Check if this is a necklace search (either explicitly or from query)
        is_necklace_search = False
        if category and category.lower() == "necklaces":
            is_necklace_search = True
        elif text_query and any(term in text_query.lower() for term in ["necklace", "necklaces", "pendant", "chain"]):
            is_necklace_search = True
            # Force category to be necklaces
            category = "necklaces"
            logger.info("Detected necklace search from query, setting category to 'necklaces'")
        ...
        search_results = await self.search_products(
            ...
            image_bytes=image_bytes,
            category=category,
            limit=limit * 2 if is_necklace_search else limit,  # Get more results for necklace searches
            min_score=min_score * 0.8 if is_necklace_search else min_score  # Lower threshold for necklace searches
        )
        
        # If this is a necklace search, prioritize necklace results
        if is_necklace_search and search_results.get('status') == 'success':
            results = search_results.get('results', [])
            
            # Prioritize items with "necklace" in the name or description
            necklace_results = []
            other_results = []
            ...
            for product in results:
                ...
                if 'necklace' in name or 'necklace' in desc or 'pendant' in name or 'pendant' in desc:
                    necklace_results.append(product)
                else:
                    other_results.append(product)
            
            # Combine results with necklaces first
            prioritized_results = necklace_results + other_results
            search_results['results'] = prioritized_results[:limit]
            search_results['message'] = f'Found {len(search_results["results"])} necklaces and similar items'
