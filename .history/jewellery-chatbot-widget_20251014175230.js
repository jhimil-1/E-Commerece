    ...
    class JewelleryChatbot {
        constructor() {
            this.config = {
                ...
                customColors: {},
                autoOpen: false,
                welcomeMessage: 'Hello! I\'m here to help you find the perfect jewellery. Ask me anything!',
                placeholderText: 'Ask about jewellery, rings, necklaces...',
        ...
        applyStyles() {
            const styles = `
                ...
                /* Color-specific product styles */
                .jewellery-chatbot-product-color-red {
                    border-left: 4px solid #ff5252;
                }
                
                .jewellery-chatbot-product-color-blue {
                    border-left: 4px solid #4285f4;
                }
                
                .jewellery-chatbot-product-color-green {
                    border-left: 4px solid #0f9d58;
                }
                
                .jewellery-chatbot-product-color-black {
                    border-left: 4px solid #212121;
                }
                
                .jewellery-chatbot-product-color-white {
                    border-left: 4px solid #f5f5f5;
                }
                
                .jewellery-chatbot-product-color-yellow {
                    border-left: 4px solid #ffeb3b;
                }
                
                .jewellery-chatbot-product-color-purple {
                    border-left: 4px solid #9c27b0;
                }
                
                .jewellery-chatbot-product-color-pink {
                    border-left: 4px solid #e91e63;
                }
                
                .jewellery-chatbot-product-color-orange {
                    border-left: 4px solid #ff9800;
                }
                
                .jewellery-chatbot-product-color-brown {
                    border-left: 4px solid #795548;
                }
                
                .jewellery-chatbot-product-color-tag {
                    position: absolute;
                    top: 10px;
                    right: 10px;
                    padding: 3px 8px;
                ...
                .jewellery-chatbot-product-color-red .jewellery-chatbot-product-color-tag {
                    background-color: #ff5252;
                }
                
                .jewellery-chatbot-product-color-blue .jewellery-chatbot-product-color-tag {
                    background-color: #4285f4;
                }
                
                .jewellery-chatbot-product-color-green .jewellery-chatbot-product-color-tag {
                    background-color: #0f9d58;
                }
                
                .jewellery-chatbot-product-color-black .jewellery-chatbot-product-color-tag {
                    background-color: #212121;
                }
                
                .jewellery-chatbot-product-color-white .jewellery-chatbot-product-color-tag {
                    background-color: #757575;
                    color: #fff;
                }
                
                .jewellery-chatbot-product-color-yellow .jewellery-chatbot-product-color-tag {
                    background-color: #ffeb3b;
                    color: #212121;
                }
                
                .jewellery-chatbot-product-color-purple .jewellery-chatbot-product-color-tag {
                    background-color: #9c27b0;
                }
                
                .jewellery-chatbot-product-color-pink .jewellery-chatbot-product-color-tag {
                    background-color: #e91e63;
                }
                
                .jewellery-chatbot-product-color-orange .jewellery-chatbot-product-color-tag {
                    background-color: #ff9800;
                    color: #212121;
                }
                
                .jewellery-chatbot-product-color-brown .jewellery-chatbot-product-color-tag {
        ...
        async sendMessage() {
            ...
            // Check if we have either text message or image file
            if (!message && !this.state.currentImageFile) return;
            ...
            // Add user message to UI (include image indicator if present)
            const displayMessage = message || (this.state.currentImageFile ? `🔍 Searching for similar jewelry...` : '');
            ...
            try {
                ...
                // Handle image search if image file is present
                if (this.state.currentImageFile) {
                    // Convert image to base64
                    const imageBase64 = await this.fileToBase64(this.state.currentImageFile);
                    
                    const data = {
                        image: imageBase64,
                        query: message || '', // Optional text query
                        session_id: this.state.sessionId,
                        limit: 5
                    };
                    
                    response = await this.makeRequest(this.apiEndpoints.imageQuery, 'POST', data);
                    
                    // Clear the image file after use
                    this.state.currentImageFile = null;
                    this.elements.filePreview.innerHTML = '';
                    this.elements.fileUpload.style.display = 'none';
                } else {
                    // Regular text search
                    const data = {
                        query: message,
                    ...
                    response = await this.makeRequest(this.apiEndpoints.chatQuery, 'POST', data);
                }
                
                // Add assistant response - ensure atomic update of both text and products
        ...
        async handleFileUpload(event) {
            ...
            // Store the file for image search
            this.state.currentImageFile = file;
            console.log('File uploaded and ready for image search:', file.name);
        }
        ...
        createProductHTML(product) {
            // Extract color information from product name and description
            const colorHighlight = this.extractColorHighlight(product);
            const colorClass = colorHighlight ? `jewellery-chatbot-product-color-${colorHighlight}` : '';
            
            return `
                <div class="jewellery-chatbot-product ${colorClass}">
                    ...
                    ${colorHighlight ? `<div class="jewellery-chatbot-product-color-tag">${colorHighlight}</div>` : ''}
        ...
        extractColorHighlight(product) {
            ...
            for (const [color, terms] of Object.entries(colorKeywords)) {
                if (terms.some(term => productName.includes(term) || productDesc.includes(term))) {
                    return color;
                }
            }
        ...
        async loadChatHistory() {
            ...
            try {
                ...
                if (this.state.messages.length === 0 && response.messages && response.messages.length > 0) {
                    // Clear existing messages (should be empty anyway)
