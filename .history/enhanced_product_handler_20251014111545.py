...
class EnhancedProductHandler:
    ...
    def filter_irrelevant_results(self, query: str, products: List[Dict[str, Any]], 
        ...
        logger.debug(f"Setting min_semantic_score for query: '{query}' (lower: '{query_lower}')")
        ...
        if min_semantic_score is None:
            ...
            if any(word in query_lower for word in headphones_words):
                ...
                logger.debug(f"Headphones detected! Words found: {[word for word in headphones_words if word in query_lower]}")
                ...
                logger.debug(f"Phone detected, setting min_semantic_score to 0.05")
                ...
                logger.debug(f"Electronics detected, setting min_semantic_score to 0.02")
                ...
                logger.debug(f"Jewelry detected, setting min_semantic_score to 0.1")
            else:
                # Standard threshold for clothing/jewelry
                min_semantic_score = 0.3
                logger.debug(f"No specific category detected, setting min_semantic_score to 0.3")
