...
def test_auth_flow():
    ...
    # Test token validation by attempting an authenticated endpoint
    headers = {"Authorization": f"Bearer {token}"}
    
    # Test with chat session creation endpoint (requires auth)
    response = session.post("http://localhost:8000/chat/sessions", headers=headers)
