...
async def debug_user_lookup():
    ...
    for user_id in test_user_ids:
        ...
        try:
            user_by_objectid = await db.users.find_one({"_id": ObjectId(user_id)})
        ...
        user_by_userid = await db.users.find_one({"user_id": user_id})
        ...
        user_by_username = await db.users.find_one({"username": user_id})
